...
Before moving onto the chi-squared test, we will first check that the overlaps between cold response genes and selected genes were not due to both datasets being biased towards high expression genes.   
```{r}
...
Gene expression of cold-response genes is not higher than expression of genes that don't respond to cold. Now we will conduct a chi-squared test to compare the proportion of genes that show evidence of selection (un-adjusted p value less than 0.05) in cold-response genes compared with other genes.
...
run_chi_sq <- function(mytissue, myPC,sigGenes, notsigGenes, myp = 0.05) {
  ...
  #chitest = chisq.test(chitbl, simulate.p.value = TRUE, B=10000) ##run the test
  chitest = fisher.test(chitbl)
...
```{r}
#test <- fread("../output/cold-chisq-table.txt")
```


Make bonferroni correction 
```{r}
apply(newresults, 2, method="bonferroni",p.adjust) 
```
