...
Now will will add a column to our that indicated if the gene is differentially expressed in cold and then divide into two dataframes.  
```{r}
Cold$Sig <- FALSE
Cold[is.na(Cold)] <- NA
...
cold_sig <- subset(Cold, Cold$Sig == TRUE)
cold_nonsig <- subset(Cold, Cold$Sig == FALSE)
```

Finally we can do the Chi-squared test  
```{r}
P_vals <- t(P_values_ind) # For other tissues you can load the tables of P-values for individual and combined from "../output/Selection_on_Expression_of_Cold_Expression_Genes/" and join them together
