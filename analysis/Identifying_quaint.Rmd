...
#pcplot <- ggplot(data=pop_dat, aes(x = pca, y= pcb , color=Subpopulation)) + scale_colour_manual(values = col, labels=mylab) + xlab(paste("PC",pca)) +  ylab(paste('PC',pcb)) + theme_bw() + theme(panel.grid.major = element_blank(), text = element_text(size=15), panel.grid.minor = element_blank(), axis.title.y = element_text(size=18), axis.title.x  = element_text(size=18), legend.position = "right", legend.title = element_text(size = 12), legend.text = element_text(size = 20), plot.title = element_text(hjust = 0.5)) + geom_point(size = 3.5)
...
par(mar=c(5,5,2,2), mfrow=c(4,2))
sapply(alltissues, function(x){makePCPlot(x, 1, 2)})

legend('center', levels(as.factor(pop_dat$Subpopulation)),col = mycol, pt.lwd=3,bty="n", pch=1)
